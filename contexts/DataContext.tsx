...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  const initializeData = () => {
    ...
    const mockProducts: Product[] = [
      ...
      },
      {
        ...
        isActive: true,
        description:
          'Hambúrguer artesanal com carne premium e ingredientes frescos',
      },
      {
        id: '3',
        name: 'Salada Caesar',
        ...
        isActive: true,
        description:
          'Salada caesar tradicional com frango grelhado e molho especial',
      },
    ];
    ...
    const mockCampaigns: Campaign[] = [
      {
        ...
        message:
          '🍕 Promoção especial! 20% off em todas as pizzas neste fim de semana!',
        sentCount: 234,
        openRate: 78,
        clickRate: 23,
        redeemRate: 12,
      ...
      },
      {
        ...
        message:
          'Sentimos sua falta! Volte e ganhe 50% de desconto no seu próximo pedido.',
        sentCount: 0,
        openRate: 0,
        clickRate: 0,
        redeemRate: 0,
    ...
    const mockAlerts: Alert[] = [
      {
        ...
        message:
          'João Santos não visita há 15 dias. Considere enviar uma campanha de reativação.',
  ...
  const dashboardData: DashboardData = {
    ...
    inactiveCustomers: customers.filter((c) => c.classification === 'inactive')
      .length,
    ...
    averagePoints:
      customers.length > 0
        ? Math.round(
            customers.reduce((sum, c) => sum + c.points, 0) / customers.length
          )
        : 0,
    monthlyData: [
  ...
  const addCustomer = (
    ...
    const newCustomer: Customer = {
      ...
      loyaltyTier: {
        ...
        nextTier: 'Silver',
        pointsToNext:
          user?.currentRestaurant.settings.loyaltyProgram.silverThreshold ||
          200,
        benefits: ['Pontos por compra'],
      },
    };
  ...
  const addCampaign = (
    campaignData: Omit<Campaign, 'id' | 'createdAt' | 'performance'>
  ) => {
  ...
  const updateCampaign = (id: string, campaignData: Partial<Campaign>) => {
    setCampaigns(
      campaigns.map((c) => (c.id === id ? { ...c, ...campaignData } : c))
    );
  };
  ...
  const updateLoyaltyRule = (id: string, ruleData: Partial<LoyaltyRule>) => {
    setLoyaltyRules(
      loyaltyRules.map((r) => (r.id === id ? { ...r, ...ruleData } : r))
    );
  };
  ...
  const generateSmartCoupon = (customerId: string): SmartCoupon | null => {
  ...
  const sendAutomatedCampaign = (ruleId: string, customerId: string) => {
    ...
    console.log(
      `Enviando campanha automatizada - Regra: ${ruleId}, Cliente: ${customerId}`
    );
  };
  ...
  const getCustomerSegmentation = (): CustomerSegmentation => {
    return {
      new: customers.filter((c) => c.classification === 'new').length,
      recurrent: customers.filter((c) => c.classification === 'recurrent')
        .length,
  ...
  const getProductPerformance = (): ProductPerformance[] => {
    return products
      .map((p) => ({
        ...p,
  ...
  const refreshAlerts = () => {
    ...
    customers.forEach((customer) => {
      if (
        customer.daysSinceLastVisit > 14 &&
        customer.classification !== 'inactive'
      ) {
        newAlerts.push({
