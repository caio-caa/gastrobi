...
function KitchenModule() {
  ...
  const [kitchenOrders, setKitchenOrders] = useState<ReturnType<typeof getKitchenOrders>>([]);
  ...
  const groupedOrders = filteredOrders.reduce(
    (acc: Record<string, typeof filteredOrders>, order) => {
      const key =
        order.orderType === 'comanda'
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600">
            ...
            {filteredOrders.length === 1 ? 'item' : 'itens'}
        ...
        <div className="flex items-center space-x-2">
          ...
          <select
            value={filter}
            onChange={(e) =>
              setFilter(e.target.value as 'all' | 'pending' | 'preparing' | 'ready')
            }
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          return (
            <div
              key={status}
              className={`p-4 rounded-lg ${bg} border border-gray-100`}
            >
                    ...
                    <div className="flex space-x-2">
                      ...
                      {order.status === 'preparing' && (
                        <Button
                          size="sm"
                          onClick={() =>
                            handleStatusUpdate(order.orderId, order.id, 'ready')
                          }
                          icon={CheckCircle}
        ...
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">
            {filter === 'all'
              ...
              : `Nenhum item ${getStatusText(filter).toLowerCase()}`}
          </h3>
