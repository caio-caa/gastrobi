...
interface POSContextType {
  ...
  // Orders
  orders: POSOrder[];
  addOrder: (order: Omit<POSOrder, 'id' | 'createdAt' | 'updatedAt'>) => string;
  updateOrder: (id: string, updates: Partial<POSOrder>) => void;
  updateOrderItem: (orderId: string, itemId: string, updates: Partial<POSOrderItem>) => void;
...
export function POSProvider({ children }: { children: React.ReactNode }) {
  ...
  const addOrder = (orderData: Omit<POSOrder, 'id' | 'createdAt' | 'updatedAt'>): string => {
  ...
  const updateOrderItem = (orderId: string, itemId: string, updates: Partial<POSOrderItem>) => {
    setOrders(prev => prev.map(order => 
      order.id === orderId 
        ? {
            ...order,
  ...
  const closeComanda = (number: string) => {
    ...
    if (comanda?.tableNumber) {
      updateTable(comanda.tableNumber, { status: 'available', waiter: undefined, currentOrder: undefined });
    }
    notifyUpdates();
  };

  const getKitchenOrders = (): POSOrderItem[] => {
    return orders
  ...
  const calculateDeliveryFee = (neighborhood: string): number => {
    return deliveryFees[neighborhood] || 15.00; // Taxa padrão
  };
