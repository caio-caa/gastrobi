...
import { ClipboardList, Plus, Minus, Send, Clock, CheckCircle, User } from 'lucide-react';
import { usePOS } from '../../contexts/POSContext';
import Button from '../ui/Button';

function ComandaModule() {
  const { products, addOrder, createComanda, comandas, tables, subscribeToUpdates } = usePOS();
            ...
            <div className="border-t bg-white p-4 space-y-3">
              <div className="max-h-40 overflow-y-auto space-y-2">
                {cart.map(item => (
                  <div key={item.productId} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg">
                    ...
                    <button
                      onClick={() => setShowObservations(showObservations === item.productId ? '' : item.productId)}
              ...
              {showObservations && (
                <div className="p-3 bg-yellow-50 rounded-lg">
                  ...
                  <input
                    type="text"
                    value={cart.find(item => item.productId === showObservations)?.observations || ''}
                    onChange={(e) => updateObservations(showObservations, e.target.value)}
              ...
              <div className="flex justify-between items-center py-2 border-t">
                ...
                <span className="text-sm text-gray-600">{cart.length} {cart.length === 1 ? 'item' : 'itens'}</span>
              </div>
